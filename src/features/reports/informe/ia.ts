 ...
 *        variaciones y los umbrales los calcula `analisis.ts` sobre los datos y
 *        cuatro operaciones. Al modelo se le entrega el expediente ya cerrado y
 *        se le prohíbe expresamente añadir números que no estén en él.
 *
 ...
 * decisión de quien lo firma: el documento es del servicio de mantenimiento y
 * habla del estado del campus, no de las herramientas con las que se preparó. El
 * rastro queda donde tiene que quedar —en `reports.params` y en la pantalla de
 * Informes—, no en el papel que se lleva a una reunión.
 *
 ...
 *
 * Esto lo llama el NAVEGADOR de quien pide el informe, no un servidor. La
 * petición va directa a Google con la clave del despliegue; no hay proxy en
 * medio y por eso no hay nada que desplegar para que funcione.
 *
 * Contrato de la API (v1beta, `generateContent`):
 *   - la clave va en la cabecera `x-goog-api-key`, nunca en la URL —la URL
 *     acaba en el historial del navegador y en los registros de cualquier proxy
 *     que haya en medio—;
 ...
 */
export const THINKING_POR_DEFECTO = 'high'
const THINKING_VALIDOS = ['minimal', 'low', 'medium', 'high'] as const

/** El extremo de la API de Google. */
const API = 'https://generativelanguage.googleapis.com/v1beta/models'

/**
 * Tope de espera.
 *
 * Más generoso que el del worker (60 s) porque aquí hay una persona mirando la
 * pantalla y viendo el progreso, no un cron: lo que no puede pasar es que se
 * rinda antes de tiempo con el razonamiento alto y un periodo largo.
 */
 ...
 *
 * Aquí ya no hay variables de entorno que consultar ni prioridades que resolver:
 * la clave es la que haya, venga de `app_config` o del propio dispositivo. Esa
 * simplificación es el motivo de todo este cambio — la clave vacía de una
 * variable declarada «por si acaso» llegó a anular durante semanas la que un
 * administrador había pegado en la aplicación, y el registro decía «sin clave»
...
export function configurarIA(ajustes: Ajustes = {}): OpcionesIA | null {
  ...
  const thinking = (ajustes.thinking ?? THINKING_POR_DEFECTO).trim()
  return {
    clave,
    modelo: (ajustes.modelo ?? MODELO_POR_DEFECTO).trim() || MODELO_POR_DEFECTO,
 ...
 * medio de un documento firmado, y eso delata la costura más que cualquier otra
 * cosa.
 */
...
async function pedir(cuerpo: unknown, o: OpcionesIA): Promise<Respuesta> {
  ...
  const r = await fetch(`${API}/${encodeURIComponent(o.modelo)}:generateContent`, {
    ...
    headers: {
      ...
      // termina en el historial del navegador y en los registros de cualquier
...
export interface Redaccion {
  /** El análisis redactado, o `null` si no se pudo. */
  lectura: Lectura | null
  /** Por qué no se pudo, para poder decirlo en la pantalla. `null` si salió bien. */
  motivo: string | null
}

/**
 * Redacta el análisis, o dice por qué no ha podido.
 *
 * Nunca lanza. Un fallo de la IA no puede impedir que salga el informe: el
 * documento se emite igual con la redacción calculada.
 *
 * Lo que sí hace, y el worker no hacía, es DEVOLVER el motivo. Allí acababa en
 * un `console.warn` del contenedor, que es donde nadie mira; aquí hay una
 * persona delante de la pantalla esperando su informe, y «ha salido con el
 * análisis calculado porque la clave no tiene permiso» es una frase que
 * resuelve el problema en vez de dejarlo abierto.
 */
export async function redactar(
  ...
  let ultimo = 'no se ha llegado a preguntar'
  ...
  return { lectura: null, motivo: ultimo }
}
