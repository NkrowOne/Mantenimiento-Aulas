...
async function comprobacionesDe(inspectionId: string): Promise<RevisionCheckRow[]> {
  const { data, error } = await supabase
    .from('inspection_check_detail')
    .select('*')
    .eq('inspection_id', inspectionId)
  if (error) throw error
  return ordenarComprobaciones((data ?? []) as RevisionCheckRow[])
}
...
export function RevisionesAnteriores({
  ...
  const { data, isPending, isError, refetch } = useQuery({
    ...
    queryFn: async (): Promise<RevisionRow[]> => {
      const { data: filas, error } = await supabase
        ...
        .select('*')
        // Solo las cerradas. La vista sirve también los borradores —el listado
        // completo tiene un filtro para ellos— y aquí no pintan nada: una
        // revisión a medias no es una visita que se pueda leer ni corregir.
        .eq('room_id', roomId)
        .eq('status', 'completa')
        .order('occurred_at', { ascending: false })
        .limit(LIMITE)
      if (error) throw error
      return (filas ?? []) as RevisionRow[]
    },
  })
  ...
  const preparar = useMutation({
    mutationFn: async (r: RevisionRow): Promise<Correccion> => {
  ...
  return (
    <li className="card p-4">
      ...
      <p className="mt-2 font-mono text-xs text-muted">
        {[
          vigente.total > 0
            ? `${vigente.total} comprobacion${vigente.total === 1 ? '' : 'es'}`
            : null,
          vigente.na > 0 ? `${vigente.na} n/a` : null,
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        >
          {viendo !== null
            ? 'Ocultar el detalle'
            : vigente.total > 0
              ? `Ver las ${vigente.total} comprobaciones`
              : 'Ver el detalle'}
        </button>
...
function Comprobaciones({ inspectionId }: { inspectionId: string }): React.ReactElement {
  ...
  return (
    <ul className="divide-y divide-line-soft">
      {(data ?? []).map((c) => {
        const estilo = RESULTADO[c.result]
        ...
        return (
          <li key={c.id} className="flex items-start gap-3 py-2">
            ...
            <span
              className={`shrink-0 rounded-tag px-2 py-0.5 text-xs font-semibold ${estilo.chip}`}
            >
              {estilo.etiqueta}
