...
export function FotosDeRevision({
  ...
  /*
   * Y las que esperan en el dispositivo.
   *
   * De la cola sale la FILA DE ESTADO, que ya no lleva los bytes dentro: viven
   * en `photoBlobs` y se piden con `leerBytesDeFoto`, que reconstruye el `Blob` a
   * partir del `ArrayBuffer`. La separación no es un detalle de organización —es
   * lo que evita que WebKit pierda el fichero de respaldo de un Blob guardado en
   * IndexedDB—, y leer aquí `p.blob` sería leer el hueco de las versiones
   * anteriores: vacío en cuanto el arranque las traslada.
   */
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      for (const p of pendientes) {
        ...
        // Sin bytes no hay nada que enseñar: es una foto cuyos datos no están en
        // este dispositivo, y la cola ya la marca como que hay que repetirla.
        if (!bytes) continue
        nuevas.push({
          id: p.id,
      ...
      // El efecto puede haberse limpiado mientras se leían los bytes: las URLs
      // que se acaban de crear no las va a revocar nadie más.
      if (cancelado) {
        for (const f of nuevas) URL.revokeObjectURL(f.url)
        return
      }
