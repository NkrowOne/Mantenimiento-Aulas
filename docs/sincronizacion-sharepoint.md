...
| **3f** | **Hecho.** La hoja `PCs STOCK <año>`: los ordenadores de repuesto por número de serie, en `stock_units`. Se sincroniza como las demás (identidad por serial, columna `Situación` escrita por la app: «En almacén», «Instalado en 2.1 C · 08/09/2026», «Baja»), se estrena sola si el libro no la trae, y en el Almacén de la aplicación hay una sección para darlos de alta, instalarlos en un aula —crea el equipo con su número de serie, retira el que hubiera y descuenta la bolsa— o darlos de baja | ✅ |
