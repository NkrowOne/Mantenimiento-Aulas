...
> **Hecho.** (a) La ubicación se guarda en `db.meta['ultima-vista']` y se rehidrata al desbloquear; también el orden elegido de la lista (`orden-salas`), que una recarga devolvía a «Por planta». «Volver» de la ficha devuelve al sitio del que se salió —lista, edificios o la revisión con su corrección intacta— y la hoja de placas vuelve a la ficha si se abrió desde ella. (b) `src/lib/historial.ts`: cada pantalla hacia dentro y cada capa que se monta encima —hoja de acciones, ficha de revisión, visor de fotos, cámara— deja una entrada en el historial, y el gesto atrás cierra la de arriba por el mismo camino que su botón «Volver»; desde otra pestaña vuelve a Revisar. La profundidad va en `history.state`, así que una recarga con pantallas abiertas vuelve a la raíz en vez de dejar entradas vacías.
